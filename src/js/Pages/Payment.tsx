...
const Payment = () => {
	...
	return (
		<div className="mx-[16rem] lg:mx-[0rem]">
			...
			<div>
				<p className="font-bodyalt font-[600] text-[22rem] leaing-[30rem] text-[#1F2117] mb-[24rem] lg:font-body lg:font-[600] lg:text-[26rem] lg:leading-[30rem] lg:text-[#1F2117] lg:mb-[32rem]">
					История платежей
				</p>
				<div className="w-full flex flex-col lg:w-full lg:flex lg:flex-col lg:mb-[0rem]">
					...
					{history?.map((e, i) => {
						let typeSub;
						let trial;
						e.paymentMethod === 'internal'
							? (typeSub = 'Внутренний')
							: e.paymentMethod === 'external'
							? (typeSub = 'Apple')
							: (typeSub = 'Бесплатная');
						...
						return (
							<div
								key={i}
								className={`grid grid-rows-7 py-[18rem] font-bodyalt font-[400] text-[#1F2117] text-[15rem] leading-[21rem] lg:grid lg:grid-rows-1 lg:grid-cols-[repeat(2,160rem)_200rem_repeat(4,160rem)] lg:py-[18rem] lg:font-bodyalt lg:font-[400] lg:text-[#1F2117] lg:text-[18rem] lg:leading-[21rem]
              ${history.length - 1 !== i ? ' border-b-[1px] border-[#E4E4E4]' : ''}`}>
								<div className="flex flex-row justify-between lg:mb-[0rem] lg:block">
									...
									<div className="font-bodyalt font-[400] text-[14rem] leaiding-[17rem] text-[#1F2117] lg:text-[18rem]">
										{new Date(e && e.createdAt ? e.createdAt : '').toLocaleDateString()}
									</div>
								</div>
								<div className="flex flex-row justify-between">
									...
									<div className="font-bodyalt font-[400] text-[14rem] leaiding-[17rem] text-[#1F2117] lg:text-[18rem] whitespace-pre text-end lg:text-start">{`${typeSub}\n${trial}`}</div>
								</div>
								<div className="flex flex-row justify-between">
									<div className="font-bodyalt font-[400] text-[14rem] leading-[19rem] text-[#AAAAAA] lg:hidden">
										Описание
								...
								<div className="flex flex-row justify-between">
									...
									<div className="font-bodyalt font-[400] text-[14rem] leaiding-[17rem] text-[#1F2117] lg:text-[18rem] whitespace-pre text-end lg:text-start">
										{e && e.paidPeriodStart
					...
					{[...ss1].map((e, i) => {
						return (
							<div
								...
								key={i}
								className="flex flex-row gap-[16rem] cursor-pointer w-max">
								<div
									className={`font-bodyalt text-[16rem] leading-[19rem]  mr-[16rem]
                ${
									currentPage === e
										? ' font-[700] text-[#1F2117] border-b-[1rem] border-[#1F2117]'
										: ' font-[400] text-[#AAAAAA]'
								}`}>
									{e === 1 && currentPage > 3
										? `${1}  ...`
										: e === lastPage.current && currentPage < lastPage.current - 2
										? `... ${lastPage.current}`
										: e}
								</div>
							</div>
				...
				<div
					className={`text-[16rem] lg:hidden 
      ${currentPage && currentPage >= lastPage.current ? ' hidden' : ''}`}
					onClick={() =>
						currentPage && currentPage >= lastPage.current
							? ''
							: setCurrentPage((prev) => {
									if (prev) {
										return prev + 1;
