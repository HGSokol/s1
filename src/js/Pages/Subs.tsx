...
import { ContinueSubPopup } from '../components/ContinueSubPopup';
...
const Subs = () => {
	...
	const [activeContinueSubPopup, setActiveContinueSubPopup] = useState(false);
	const [sub, setSub] = useState<SubscribeType[] | null>(null);
	...
	return (
		<div className="mx-[16rem] lg:mx-[0rem]">
			{!didMount ? (
				<div className="mt-[200rem] lg:mt-[0rem]  flex items-center justify-center">
					<Spinner />
				</div>
			) : (
				<>
					{activeSub?.isFromApple ? (
						<>
							<div className="w-full flex flex-row relative mb-[24rem] lg:hidden">
								<div
									className="absolute translate-y-[20.5rem]"
							...
							<div className=" mb-[24rem] flex flex-col gap-[24rem] lg:flex-row lg:justify-between lg:mb-[48rem] lg:w-[472rem]">
								<div className="w-full flex justify-between items-center lg:items-start flex-row lg:flex-col lg:justify-start">
									...
									<p className="font-bodyalt font-[400] text-[14rem] leading-[17rem] text-[#777872] lg:font-bodyalt lg:font-[400] lg:text-[16rem] lg:leading-[19rem] lg:text-[#777872]">{`${
										...
										activeSub?.duration === 1
											? 'мес.'
											: activeSub?.duration === 3
											? '3 мес.'
											: 'год'
									}`}</p>
								</div>
						...
						<>
							...
							{activeContinueSubPopup ? (
								<div className="absolute z-[1000] top-0 left-0 w-[100%] h-[100%] bg-gray-100/50">
									<div className="absolute z-[1000] left-[50%] top-[50%] -translate-x-[50%] -translate-y-[50%]  ">
										<ContinueSubPopup setActiveContinueSubPopup={setActiveContinueSubPopup} />
									</div>
								</div>
							) : null}
								...
								<div>
									...
									<div className="w-full lg:w-[356rem]">
										{activeSub && activeSub.error ? (
											<div className="text-[16rem] mb-[20rem] ">
												<div className="mb-[16rem] lg:mb-[24rem]">
													<div className="flex flex-row gap-[16rem]">
														<div>
															<svg
																className="h-[30rem] w-[30rem] lg:h-[36rem] lg:w-[36rem]"
																viewBox="0 0 36 36"
																fill="none"
																xmlns="http://www.w3.org/2000/svg">
																<path
																	fillRule="evenodd"
																	clipRule="evenodd"
																	d="M18 4.65354C10.629 4.65354 4.65354 10.629 4.65354 18C4.65354 25.371 10.629 31.3465 18 31.3465C25.371 31.3465 31.3465 25.371 31.3465 18C31.3465 10.629 25.371 4.65354 18 4.65354ZM3 18C3 9.71573 9.71573 3 18 3C26.2843 3 33 9.71573 33 18C33 26.2843 26.2843 33 18 33C9.71573 33 3 26.2843 3 18ZM18 9.75C18.4142 9.75 18.75 10.0858 18.75 10.5V19.5C18.75 19.9142 18.4142 20.25 18 20.25C17.5858 20.25 17.25 19.9142 17.25 19.5V10.5C17.25 10.0858 17.5858 9.75 18 9.75ZM18.8394 23.2499C18.6285 23.0389 18.3423 22.9204 18.0439 22.9204C17.7456 22.9204 17.4594 23.0389 17.2485 23.2499C17.0375 23.4609 16.9189 23.747 16.9189 24.0454C16.9189 24.3438 17.0375 24.6299 17.2485 24.8409C17.4594 25.0519 17.7456 25.1704 18.0439 25.1704C18.3423 25.1704 18.6285 25.0519 18.8394 24.8409C19.0504 24.6299 19.1689 24.3438 19.1689 24.0454C19.1689 23.747 19.0504 23.4609 18.8394 23.2499Z"
																	fill="#CB1D1D"
																/>
															</svg>
														</div>
														<div className="text-[#CB1D1D]">
															Последняя попытка списания была неуспешной
														</div>
													</div>
												</div>
												<div className=" mb-[24rem] w-full flex flex-col gap-[24rem] lg:flex-row lg:justify-between lg:mb-[20rem] lg:w-full">
							...
							<div className="mb-[30rem] h-max flex flex-col lg:grid lg:grid-cols-3 gap-[24rem] lg:w-[1370rem] lg:gap-[40rem]">
								{sub?.map((e, i) => {
									return (
										<div key={i}>
											<SubCard
